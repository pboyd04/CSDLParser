...
Metadata.prototype.parseSchema = function(schema) {
  ...
  var simpleTypes = schema.find('.//*[local-name()="TypeDefinition"]');
  for(var i = 0; i < simpleTypes.length; i++)
  {
    var simpleType = simpleTypes[i];
    var name = simpleType.attr('Name').value();
    this.TypeDefinitions[namespace+"."+name] = new TypeDefinition(this, simpleType);
  }
  ...
  var functions = schema.find('.//*[local-name()="Function"]');
  for(var i = 0; i < functions.length; i++)
  {
    var name = functions[i].attr('Name').value();
    this.Functions[namespace+"."+name] = new Function(this, functions[i]);
  }
}
